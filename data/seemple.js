export default [{
  name: 'seemple',
  description: 'A simple JavaScript framework for single page applications creation. It fills the gap between a junior and a senior because of simple and intuitive API based on JavaScript classes and accessors.',
  npm: 'seemple',
  tags: ['highlight', 'browser'],
}, {
  name: 'seemple-router',
  githubLink: 'https://github.com/finom/seemple/tree/master/packages/router',
  description: 'A router library for Seemple.js. The library turns on two-way data binding between object properties and parts of an URL.',
  npm: 'seemple-router',
  tags: ['browser'],
}, {
  name: 'seemple.js.org',
  description: 'A website with detailed three-lingual documentation for Seemple.js. It\'s built automatically via custom Webpack plugin script from JSDoc files.',
  link: 'https://seemple.js.org',
  tags: ['website'],
}, {
  name: 'seemple-parse-form',
  githubLink: 'https://github.com/finom/seemple/tree/master/packages/parse-form',
  description: 'The function binds named HTML form fields (input, select, textarea etc) contained at given HTML form to corresponding object properties.',
  npm: 'seemple-parse-form',
  tags: ['browser'],
}, {
  name: 'seemple-examples-and-tutorials',
  description: 'The repository contains all the main tutorials and some example applications (Contact List, Markdown Editor, Tree View, SoundCloud player, TodoMVC).',
  tags: ['example'],
}];
